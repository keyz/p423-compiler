...
(library (Framework wrappers)
  (export
    ...
    expose-allocation-pointer/wrapper ;; [a8 mod]
    ...
    expose-memory-operands/wrapper ;; [a8 mod]
  ...
  (import
    ...
    (Framework GenGrammars l26-expose-allocation-pointer) ;; [a8 mod]
    ...
    (Framework GenGrammars l38-expose-memory-operands) ;; [a8 mod]
...
(define pass->wrapper
  (lambda (pass)
    ...
    (case pass
      ...
      ((expose-allocation-pointer) expose-allocation-pointer/wrapper) ;; [a8 mod]
      ...
      ((expose-memory-operands) expose-memory-operands/wrapper) ;; [a8 mod]
...
;; expose-allocation-pointer/wrapper  [a8 mod]
;;-----------------------------------
(define-language-wrapper expose-allocation-pointer/wrapper ;; do I need to wrap it in a `()` here?
  (x)
  (environment env)
  (define frame-size ,(compute-frame-size x))
  ,return-point-complex
  ,new-frames ;; no more `,alloc`
  ,set!
  (import
    (only (Framework wrappers aux)
      handle-overflow letrec locals true false nop))
